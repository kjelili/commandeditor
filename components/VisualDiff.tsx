...
export const VisualDiff: React.FC<Props> = ({ 
  ...
  const comparePDFs = async () => {
    ...
    try {
      const pdfA = await pdfjs.getDocument({ standardFontDataUrl: '/pdf-standard-fonts/', data: pdfABytes.slice() }).promise;
      const pdfB = await pdfjs.getDocument({ standardFontDataUrl: '/pdf-standard-fonts/', data: pdfBBytes.slice() }).promise;
